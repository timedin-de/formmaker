 ...
 * group headings (only when the group holds at least one answer) followed by
 * their answered questions, indented by nesting depth. Sections are treated as
...
export function buildReceipt(form: FormDefinition, submission: Submission): ReceiptBlock[] {
  ...
  form.pages.forEach((page) => walk(page.elements, 0));
  return blocks;
}
